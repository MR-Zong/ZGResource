...
- (void)setTextColor:(UIColor *)textColor
{
    ...
    if(_textColor != textColor)
    {
        ...
        if(_attributedText)
        {
            ...
            [self setURLAttributesWithAttributedText:_attributedText];
            [self redrawText];
        }
    }
}
...
- (void)setURLAttributes:(NSDictionary *)URLAttributes
{
    if(!URLAttributes)
    {
        URLAttributes = [NSDictionary dictionaryWithObjectsAndKeys:(id)[_UIKitTintColor_ CGColor], (NSString*)kCTForegroundColorAttributeName, [NSNumber numberWithBool:YES], (NSString *)kCTUnderlineStyleAttributeName, nil];
    }
...
- (NSMutableAttributedString*)defaultAttributedTextFromString:(NSString*) string
{
    ...
    [self setURLAttributesWithAttributedText:attributedText];
...
- (void)addSelectableRange:(NSRange) range
{
    if(range.location + range.length <= self.text.length)
    {
        ...
        [_attributedText addAttributes:self.URLAttributes range:range];
        [self redrawText];
    }
}
...
///设置URL样式
- (void)setURLAttributesWithAttributedText:(NSMutableAttributedString*) attributedText
{
    if(self.identifyURL && attributedText)
    {
        ...
        {
            [attributedText addAttributes:self.URLAttributes range:result.rangeValue];
        }
    }
}
