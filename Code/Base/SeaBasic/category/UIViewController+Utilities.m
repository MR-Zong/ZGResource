    ...
    if(self.loading != loading)
    {
        ...
        if(loading)
        {
            ...
            if(!self.loadingIndicator)
            {
                ...
                SeaLoadingIndicator *indicator = [[SeaLoadingIndicator alloc] initWithFrame:CGRectMake(0, 0, _width_, self.view.height) title:@"正在载入..."];
...
- (void)setHasNoMsgViewHidden:(BOOL) hidden msg:(NSString*) msg
{
    if(!hidden && !self.hasNoMsgView)
    {
        ...
        if([self isKindOfClass:[SeaTableViewController class]])
        {
            SeaTableViewController *t = (SeaTableViewController*)self;
            
            if(t.tableView == nil || (t.tableView.tableFooterView && t.tableView.tableHeaderView))
            {
                [self.view addSubview:label];
            }
            else
            {
                label.frame = t.tableView.frame;
                CGFloat height = t.tableView.tableHeaderView.height;
                label.height -= height;
                t.tableView.tableFooterView = label;
            }
        }
        else if ([self isKindOfClass:[SeaCollectionViewController class]])
        {
            SeaCollectionViewController *c = (SeaCollectionViewController*)self;
            if(c.collectionView)
            {
                label.frame = c.collectionView.frame;
            }
        }
    ...
    {
        SeaTableViewController *t = (SeaTableViewController*)self;
        t.tableView.tableFooterView = hidden ? nil : self.hasNoMsgView;
    }
    else
    {
...
- (void)setupDefaultNavigationBar
{
    ...
    dic = [NSDictionary dictionaryWithObjectsAndKeys:self.iconTintColor, NSForegroundColorAttributeName, [UIFont fontWithName:MainFontName size:20.0], NSFontAttributeName, nil];
    ...
    navigationBar.tintColor = self.iconTintColor;
