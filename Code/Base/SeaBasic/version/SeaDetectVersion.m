...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    if(self.receiveData != nil)
    {
        ...
        if([dic isKindOfClass:[NSDictionary class]])
        {
            ...
            self.localVersion = [localDic stringForKey:appLocalVersion];
            
            if(resArray.count > 0)
            {
                NSDictionary *valueDic = [resArray firstObject];
                self.newestVersion = [valueDic stringForKey:appVersionKey];
                ...
                {
                    self.trackViewUrl = [valueDic stringForKey:appURL];
                }
            }
        }
    }
